...
get_raw_data <- function(jwt, cache = FALSE, auth = FALSE, local = FALSE) {
...
data_add_processed <- function(rawdata) {
    ...
    print(paste0("    ---- Transforming TUD took: ", (proc.time() - ptm)['elapsed'], " seconds."))
    ptm <- proc.time()
    ...
    print(paste0("    ---- Transforming MAQ took: ", (proc.time() - ptm)['elapsed'], " seconds."))
    ptm <- proc.time()
    chr_pr = process_chronicle(rawdata) %>% mutate(chr_present = TRUE)
    print(paste0("    ---- Transforming chronicle took: ", (proc.time() - ptm)['elapsed'], " seconds."))
    ptm <- proc.time()
    bigdataset = tud_sm %>%
    ...
    select(-c(study.x, study.y, table_access.x, table_access.y))
        print(paste0("    ---- Putting data together took: ", (proc.time() - ptm)['elapsed'], " seconds."))
...
data_get_coltypes <- function(rawdata, datasets = c("tud", "maq", "chronicle"), types = c("numeric", "factorial", "boolean")) {
    ...
    done = c(coltype$tud_activity$factorial, coltype$tud_activity$boolean, coltype$tud$factorial, coltype$tud$boolean, 
             coltype$maq$factorial, coltype$maq$boolean, coltype$chronicle$factorial, coltype$chronicle$boolean)

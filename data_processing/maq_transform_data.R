...
process_maq <- function(rawdata) {
  ...
  devices_mediauseattitudes = devices %>%
    ...
    group_by(child_id) %>%
  summarise(
    sf_maq_Q10_uses_videochat_a = sum(
  ...
  respondentdetails = recombine(list("Respondents", "RespondentDetails"), rawdata) %>%
    ...
    summarise(
      ...
      parental_marital = paste(unique(
        RespondentDetails.person.maritalstatus
      ), collapse = ", "),
  ...
  education = recombine(list("Respondents", "Education"), rawdata) %>%
    ...
    group_by(child_id) %>%
    summarise(parental_education = last(fct_relevel(parental_education, education_levels))) %>% select(parental_education, child_id)
  ...
  parents_mediation_sf <-
    ...
    summarise(
      ...
      sf_maq_Q9_mediacontent = sum(
        str_detect(ParentMediationScale.ol.parentalcontrols, "Most of the time") &
          str_detect(
          ...
          str_detect(ParentMediationScale.ol.usesratingsystem, "Most of the time"),
        na.rm = TRUE
      ) >= 2
    )
